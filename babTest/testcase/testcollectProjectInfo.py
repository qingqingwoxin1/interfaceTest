...
collectProjectInfo_xls = readExcel().get_xls('userCase.xlsx', 'collectProjectInfo')
util = Utility()
db = DB()
...
class collectProjectInfo(unittest.TestCase):
    ...
    def test_collectProjectInfo(self):
        self.chechResult()
    ...
    def chechResult(self):
        ...
        if self.case_name == 'collectProjectInfo_success':  # 如果case_name是login，说明合法，返回的code应该为200
            self.assertEqual(self.expect_code, ss['code'])
        if self.case_name == 'collectProjectInfo_error':  # 同上
            self.assertEqual(self.expect_code, ss['code'])
